...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14("upgrade_bounty"), bytes16("P_RequiredDepend")))
);
ResourceId constant P_RequiredDependencyTableId = _tableId;
...
library P_RequiredDependency {
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (uint8 resource, uint256 amount) {
    resource = (uint8(Bytes.slice1(_blob, 0)));

    amount = (uint256(Bytes.slice32(_blob, 1)));
  }
   ...
   */
  function encode(uint8 resource, uint256 amount) internal pure returns (bytes memory, PackedCounter, bytes memory) {
    bytes memory _staticData = encodeStatic(resource, amount);
