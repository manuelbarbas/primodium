...
uint256 constant ID = uint256(keccak256("system.S_ResolveBattle"));

contract S_ResolveBattleSystem is PrimodiumSystem, IOnEntitySubsystem {
  ...
  function execute(bytes memory args) public override returns (bytes memory) {
    ...
    require(!battleResultComponent.has(battleEntity), "S_ResolveBattleSystem: battle already resolved");
