...
// tables
import { P_ResourceReward, P_ResourceRewardData, P_UnitReward, P_UnitRewardData, P_RequiredObjectives, HasCompletedObjectiveComponent, P_EnumToPrototype, P_MaxLevel, Home, P_RequiredTile, P_ProducesUnits, P_RequiredBaseLevel, P_Terrain, P_AsteroidData, P_Asteroid, Spawned, DimensionsData, Dimensions, PositionData, Level, BuildingType, Position, LastClaimedAt, Children, OwnedBy, P_Blueprint, Children } from "codegen/index.sol";
...
library LibReward {
  ...
  function receiveUnitRewards(bytes32 playerEntity, bytes32 prototype) internal {
    ...
    for (uint256 i = 0; i < rewardData.unitTypes.length; i++) {
      LibUnit.updateStoredUtilities(playerEntity, rewardData.unitTypes[i], rewardData.counts[i], true);
