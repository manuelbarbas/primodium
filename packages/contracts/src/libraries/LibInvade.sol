...
// types
import { Coord, Arrival, ArrivalUnit, BattleParticipant, ESendType } from "src/types.sol";

library LibInvade {
  function invade(IWorld world, uint256 invader, uint256 rockEntity) internal {
    uint256 playerAsteroidEntity = LibEncode.hashKeyEntity(invader, rockEntity);
    OwnedByComponent ownedByComponent = OwnedByComponent(world.getComponent(OwnedByComponentID));
    require(ownedByComponent.has(rockEntity), "[Invade]: request invasion can not be done on a rock that is not owned");
