...
import { P_RawResource, P_SizeToAmount, MaxResourceCount, ResourceCount, P_GameConfig, P_GameConfigData, LastClaimedAt, Motherlode, MotherlodeData, Position, PositionData, ReversePosition, RockType } from "codegen/index.sol";
...
library LibMotherlode {
  ...
  function initMotherlode(PositionData memory position, bytes32 motherlodeEntity) internal {
    ...
    MaxResourceCount.set(motherlodeEntity, P_RawResource.get(uint8(motherlodeType)), P_SizeToAmount.get(uint8(size)));
    ResourceCount.set(motherlodeEntity, P_RawResource.get(uint8(motherlodeType)), P_SizeToAmount.get(uint8(size)));
