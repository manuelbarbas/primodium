...
// tables
import { P_RequiredObjectives, HasCompletedObjectiveComponent, P_EnumToPrototype, P_MaxLevel, Home, P_RequiredTile, P_ProducesUnits, P_RequiredBaseLevel, P_Terrain, P_AsteroidData, P_Asteroid, Spawned, DimensionsData, Dimensions, PositionData, Level, BuildingType, Position, LastClaimedAt, Children, OwnedBy, P_Blueprint, Children } from "codegen/index.sol";
...
library LibObjectives {
  function checkObjectiveRequirements(bytes32 playerEntity, bytes32 objective) internal {
    checkHasNotCompletedObjective(playerEntity, objective);
    checkHasCompletedRequiredObjectives(playerEntity, objective);
    checkObjectiveMainBaseLevelRequirement(playerEntity, objective);
    checkHasBuiltRequiredBuildings(playerEntity, objective);
  }

  function checkHasNotCompletedObjective(bytes32 playerEntity, bytes32 objective) internal {
    require(!CompletedObjective.get(playerEntity, objective), "[LibObjectives] Player has already completed objective");
  }
