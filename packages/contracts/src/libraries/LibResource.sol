...
import { P_IsStorageFull, Level, IsActive, Motherlode, OwnedMotherlodes, P_ConsumesResource, ConsumptionRate, Home, P_IsAdvancedResource, ProducedResource, P_RequiredResources, P_IsUtility, ProducedResource, P_RequiredResources, Score, P_ScoreMultiplier, P_IsUtility, P_RequiredResources, P_GameConfig, P_RequiredResourcesData, P_RequiredUpgradeResources, P_RequiredUpgradeResourcesData, P_EnumToPrototype, ResourceCount, MaxResourceCount, UnitLevel, LastClaimedAt, ProductionRate, BuildingType, OwnedBy } from "codegen/index.sol";
...
library LibResource {
    ...
    returns (uint256 totalResources, uint256[] memory resourceCounts)
  {
    ...
    for (uint8 i = 1; i < resourceCounts.length; i++) {
      if (P_IsUtility.get(i) || P_IsStorageFull.get(i)) continue;
