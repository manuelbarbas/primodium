...
library LibUpgrade {
    ...
    function checkUpgradeResourceRequirements(Uint256Component buildingComponent,Uint256ArrayComponent resourceRequirementsComponent,Uint256Component itemComponent,uint256 buildingId,uint256 buildingEntity, address playerAddress) internal view returns (bool)
    {
        ...
        require(resourceRequirementsComponent.has(LibEncode.hashFromKey(buildingId,currentLevel + 1)), "[LibUpgrade] Resource Requirements do not exist for this building level");
        uint256[] memory resourceRequirements = resourceRequirementsComponent.getValue(LibEncode.hashFromKey(buildingId,currentLevel + 1));
    ...
    function spendUpgradeResourceRequirements(Uint256Component buildingComponent,Uint256ArrayComponent resourceRequirementsComponent,Uint256Component itemComponent,uint256 buildingId,uint256 buildingEntity, address playerAddress) internal
    {
        require(buildingComponent.has(buildingEntity),"[LibUpgrade] can not upgrade building that does not exist");
        uint256 currentLevel = buildingComponent.getValue(buildingEntity);
        require(resourceRequirementsComponent.has(LibEncode.hashFromKey(buildingId,currentLevel + 1)), "[LibUpgrade] Resource Requirements do not exist for this building level");
        ...
        for(uint256 i = 0; i < resourceRequirements.length; i++)
        {
            ...
            itemComponent,LibEncode.hashFromAddress(resourceRequirements[i],
             playerAddress));
            itemComponent.set(
            LibEncode.hashFromAddress(resourceRequirements[i], playerAddress),
    ...
    function checkUpgradeResearchRequirements(Uint256Component buildingComponent,Uint256Component researchRequirmentComponent,BoolComponent researchComponent,uint256 buildingId,uint256 buildingEntity, address playerAddress) internal view returns (bool)
    {
        require(buildingComponent.has(buildingEntity),"[LibUpgrade] can not upgrade building that does not exist");
        uint256 currentLevel = buildingComponent.getValue(buildingEntity);
