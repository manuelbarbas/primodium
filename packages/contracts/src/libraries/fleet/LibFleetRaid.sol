...
library LibFleetRaid {
  ...
  function resolveBattleRaid(bytes32 raider, bytes32 target) internal {
  ...
  function calculateRaidFromWithAllies(bytes32 targetEntity, uint256 maxRaidAmount)
    internal
    returns (uint256[] memory totalRaidedResourceCounts, uint256 totalRaidedResources)
  {
    (
  ...
  function calculateRaidFrom(
    ...
    for (uint256 i = 0; i < transportables.length; i++) {
      uint256 resourceCount = ResourceCount.get(targetEntity, transportables[i]);
      if (resourceCount == 0) continue;
      uint256 resourcePortion = ((resourceCount * maxRaidAmount) / totalRaidableResources);
  ...
  function calculateRaidAllFrom(bytes32 targetEntity)
    internal
    returns (uint256[] memory totalRaidedResourceCounts, uint256 totalRaidedResources)
  {
    uint8[] memory transportables = P_Transportables.get();
    totalRaidedResourceCounts = new uint256[](transportables.length);
    for (uint256 i = 0; i < transportables.length; i++) {
      uint256 resourceCount = ResourceCount.get(targetEntity, transportables[i]);
      if (resourceCount == 0) continue;
      if (IsFleet.get(targetEntity)) {
        LibFleet.decreaseFleetResource(targetEntity, transportables[i], resourceCount);
      } else {
        LibStorage.decreaseStoredResource(targetEntity, transportables[i], resourceCount);
      }
      totalRaidedResourceCounts[i] += resourceCount;
      totalRaidedResources += resourceCount;
    }
  }
