...
bytes32 constant prototypeId = "UnitTraining";
bytes32 constant UnitTrainingPrototypeId = prototypeId;
uint256 constant LENGTH = 2;

function UnitTrainingKeys() pure returns (bytes32[] memory) {
...
function UnitTrainingPrototype(IStore store) {
  bytes32[] memory keys = UnitTrainingKeys();
