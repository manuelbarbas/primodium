...
contract ClaimFactorySystemTest is MudTest {
  ...
  function testClaimFactoryNoCraft() public {
    ...
    uint256 hashedBulletFactoryKey = LibEncode.hashKeyEntity(IronResourceItemID, bulletFactoryID);
  ...
  function testClaimFactoryCraft() public {
    ...
    assertEq(itemComponent.getValue(LibEncode.hashKeyEntity(BulletCraftedItemID, addressToEntity(alice))), 300);
  ...
  function testClaimFactoryToFactory() public {
    ...
    buildSystem.executeTyped(DebugNodeID, endPath2Coord);
    buildSystem.executeTyped(DebugNodeID, startPath2Coord);
