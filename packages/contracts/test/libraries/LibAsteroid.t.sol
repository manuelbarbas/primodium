...
contract LibAsteroidTest is PrimodiumTest {
  ...
  function testFuzzPositionByVector(uint32 distance, uint32 direction) public {
    ...
    PositionData memory destination = LibAsteroid.getPositionByVector(distance, direction);
    uint32 reverseDirection = direction + 180;
    PositionData memory origin = LibAsteroid.getPositionByVector(distance, reverseDirection);
  ...
  function testPrintPositions() public view {
    ...
    for (uint32 i = 0; i < max; i++) {
      PositionData memory coord = LibAsteroid.getPositionByVector(distance, (i * 360) / max);
      logPosition(coord);
    }
  }
}
