 ...
 *
 * This file is generated by Amplitude. To update run 'ampli pull web'
 *
...
export interface SystemAddOrderProperties {
  /**
   * Count of a resource, currently only used in Marketplace events. Logged here with identical scaling displayed in the
   * frontend.
   *
   * | Rule | Value  |
   * | ---- | ------ |
  ...
  amountIn: number;
  /**
   * Price of a resource. The raw price is stored in wei units in smart contracts, but logged here with identical
   * scaling displayed in the frontend.
   *
   * | Rule | Value  |
   * | ---- | ------ |
   * | Type | number |
...
export interface SystemBuildProperties {
  /**
   * Location of an asteroid represented as the [z] element in the Position component. This is stored as a single string
   * because the asteroid location is greater than the int32 number limit and has type BigNumber in the client.
   */
  ...
  buildingType: string;
  /**
   * Most systems take a coordinate as a parameter and read the specific building and related metadata during contract
   * execution. Even though such metadata (eg building type and level) aren't passed into the system, we fetch them
   * manually and pass them into Amplitude properties for easier analysis.
   *
  ...
  coord: [number, number];
  /**
   * Current level of the building being upgraded. If there is a duplicate event, then the user failed to upgrade the
   * building in the previous action. Also refers to the level of building expansion on an asteroid.
   *
   * | Rule | Value  |
   * | ---- | ------ |
   * | Type | number |
...
export interface SystemBuildPathProperties {
  ...
  coord: [number, number];
  /**
   * Coordinates at which a path ends or the destination where something is moved to (e.g. a building). The starting
   * coordinates are recorded by the `coord` property.
   *
   * | Rule      | Value  |
   * | --------- | ------ |
   * | Min Items | 2      |
...
export interface SystemFleetSendSystemPrimodiumSendFleetProperties {
  ...
  spaceRock: string;
  /**
   * Position of a space rock, represented by [x,y] as numbers. This property is optional for FleetSendSystem if a space
   * rock entity ID is specified instead.
   *
   * | Rule      | Value  |
   * | --------- | ------ |
   * | Min Items | 2      |
...
export interface SystemGrantRoleProperties {
  ...
  allianceName: string;
  /**
   * A member role in an alliance that has been granted. Currently only used by `system.GrantRole`. Defined as the
   * following:
   *
   *     enum EAllianceRole {
...
export interface SystemResearchProperties {
  /**
   * Name of a research objective in plaintext, as returned by `BlockIdToKey` in `constants.ts` when passing in an
   * EntityID.
   */
  researchType: string;
  /**
...
export interface SystemSendUnitsProperties {
  ...
  asteroidCoord: string;
  /**
   * Location of an asteroid represented as the [z] element in the Position component. See `asteroidCoord` for more
   * info.
   */
...
export interface SystemTakeOrderBulkProperties {
  /**
   * Counts of resources taken from each Marketplace order, currently used in `system.TakeOrderBulk`. Used in
   * conjunction with `marketplaceOrderIds`.
   *
   * | Rule      | Value  |
   * | --------- | ------ |
   * | Item Type | number |
  ...
  /**
   * An array of Marketplace order IDs, currently used in `system.TakeOrderBulk`. Used in conjunction with
...
export interface SystemTrainUnitsProperties {
   ...
   * Name of a building in plaintext, as returned by `BlockIdToKey` in `constants.ts` when passing in an EntityID.
   *
   * Note that this property is suffixed `Name` instead of `Type` for future provisions for custom names per building.
   * This is otherwise identical to `buildingType` but should only be used in `system.TrainUnits` thus far.
   */
  buildingName: string;
  /**
   * The address this transaction is from. On Amplitude, this is also tracked as the user's unique account address
...
export interface TokenMintProperties {
  ...
  tokenMintTo: string;
  /**
   * Amount of token that is transferred or minted, logged in wei units as a string. This is named `value` in line with
   * the ERC20 standard.
   */
  tokenValue: string;
  /**
