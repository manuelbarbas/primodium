...
export const BuildingImageFromType: React.FC<{ buildingType: Entity; blurred?: boolean }> = ({
  ...
  return (
    <div className={`relative flex flex-col text-sm items-center cursor-pointer min-w-[4rem] h-10`}>
      <img
        src={imageUri}
        className={`absolute pointer-events-none bottom-0 w-12 pixel-images rounded-md ${
          blurred ? "darken-[3px]" : ""
        }`}
      />
    </div>
