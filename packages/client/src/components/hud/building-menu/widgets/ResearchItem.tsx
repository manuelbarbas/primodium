...
export const ResearchItem: React.FC<{ data: ResearchItemType }> = memo(
  ({ data }) => {
    // fetch whether research is completed
    const network = useMud();
    const player = Account.use()?.value ?? SingletonID;
    const { name, levels } = data;
    const { level, mainBaseLvlReq, maxLevel, recipe } = getResearchInfo(
      data,
      player
    );
    ...
    return (
      <SecondaryCard className="flex flex-col justify-between items-center h-full">
        ...
        {error && (
          <p className="text-xs text-error animate-pulse uppercase py-1">
            {error}
          </p>
        )}
      </SecondaryCard>
