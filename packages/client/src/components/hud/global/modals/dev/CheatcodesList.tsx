...
export const CheatcodesList = ({ cheatcodes, className = "" }: { cheatcodes: Cheatcodes; className?: string }) => {
  ...
  return (
    <div className={`flex flex-col w-full h-full overflow-hidden pointer-events-auto ${className}`}>
      ...
      <div className="flex flex-col overflow-y-scroll scrollbar w-full h-full">
        {cheatcodes.map((section, sectionIndex) => (
          <div key={`section-${sectionIndex}`} className="p-1">
            ...
            >
              <h2 className="text-lg text-white font-bold w-full">{section.title}</h2>
              <div className="grid place-items-center">
                <div
                  ...
                  style={{
                    transform: `rotate(${expandedSections[section.title] ? "180deg" : "0deg"})`,
                  }}
                ></div>
              </div>
            </button>
                              ...
                              <div key={`param-input-${param.name}-${index}`} className="flex gap-3 items-center">
                                ...
                                {param.type === "dropdown" ? (
                                  ...
                                  >
                                    ...
                                    {param.dropdownOptions?.map((option, optionIndex) => (
                                      <option key={`dropdown-option-${param.name}-${optionIndex}`} value={option}>
