...
export const useClaimPrimodium = (asteroidEntity: Entity) => {
  const gameOver = components.VictoryStatus.use()?.gameOver ?? false;
  ...
  return useMemo(() => {
    if (gameOver || points === 0n || !owner || !conquestConfig || !gameConfig) return null;
  ...
  }, [conquestConfig, gameOver, gameConfig, lastConquered, owner, points, time]);
};
