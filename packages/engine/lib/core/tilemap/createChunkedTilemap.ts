...
>(
  ...
  if (
    mod(chunks.chunkSize, tileWidth) !== 0 ||
    mod(chunks.chunkSize, tileHeight) !== 0
  ) {
    throw new Error(
      "Chunks pixel size must be a multiple of tile width and height to be used with chunked tilemap"
    );
  }
  ...
  const chunkTileSize = {
    x: chunks.chunkSize / tileWidth,
    y: chunks.chunkSize / tileHeight,
  };
  ...
  function createLayers(
    ...
    return {
      ...
      defaultLayer: layers[layerConfig.defaultLayer],
    };
  }
  ...
  function createMap(chunkCoord: WorldCoord): Phaser.Tilemaps.Tilemap {
    ...
    const { defaultLayer } = createLayers(
      map,
      scene,
      topLeft.x,
      topLeft.y,
  ...
  function renderChunk(chunkCoord: ChunkCoord) {
    ...
    for (const layer of Object.keys(layerConfig.layers))
      map.forEachTile(
        (tile) => {
          const coord = {
            x: topLeftCoord.x + tile.x,
            y: topLeftCoord.y + tile.y,
          };
          const defaultIndex =
