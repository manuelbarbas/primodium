...
import { AnimatedTilemap, TileAnimation } from "./types";
import { WorldCoord } from "@latticexyz/phaserx/dist/types";
import { mod } from "@latticexyz/phaserx";
...
>(
  config: Omit<ChunkedTilemapConfig<TileKeys, string>, "tiles"> & {
  ...
  let lastStep = 0;
  const animations: {
    [key in AnimationKeys]?: { frames: TileAnimation<TileKeys>; index: number };
  } = {};

  const animatedTiles: { [key in LayerKeys]: CoordMap<AnimationKeys> } =
    {} as never;
  for (const layerKey of Object.keys(layers)) {
    animatedTiles[layerKey as LayerKeys] = new CoordMap<AnimationKeys>();
  }

  const pausedAnimations: { [key in LayerKeys]: CoordMap<AnimationKeys> } =
    {} as never;
  for (const layerKey of Object.keys(layers)) {
  ...
  function putAnimationAt(
    coord: WorldCoord,
    animationKey: AnimationKeys,
    layer: LayerKeys = defaultLayerKey
  ) {
    animatedTiles[layer].set(coord, animationKey);
  }

  function removeAnimationAt(
    coord: WorldCoord,
    layer: LayerKeys = defaultLayerKey
  ) {
  ...
  function animationStep() {
    ...
    for (const animationKey of Object.keys(animations) as AnimationKeys[]) {
      ...
      if (animation)
        animation.index = mod(animation.index + 1, animation.frames.length);
    }
